...
# inference results from Ref-Thinker, the correponding data are from Ref-AVSBench
# JSONL_PATH only used to indicate the path for saving log, do not need the inference results actually
...
BOX_THRESHOLD = 0.1 
TEXT_THRESHOLD = 0.25 
USE_ORI_REF_TEXT = True # not modify, cannot be True when using USE_F_OBJECT
USE_F_OBJECT = False #! not modify
...
if USE_F_OBJECT:
    ...
    metric_log_path = os.path.join(os.path.dirname(JSONL_PATH), f"s_object_eval_results_v2_boxthre{BOX_THRESHOLD}_textthre{TEXT_THRESHOLD}.json")

if USE_ORI_REF_TEXT:
    OUTPUT_DIR = os.path.join(os.path.dirname(JSONL_PATH), "video_batch_infer_ori_ref")
    metric_log_path = os.path.join(os.path.dirname(JSONL_PATH), f"direct_ref_eval_results_v2_boxthre{BOX_THRESHOLD}_textthre{TEXT_THRESHOLD}_reproduce.json")
