...
# inference results from Ref-Thinker, the correponding data are from R^2-AVSBench
JSONL_PATH = "epochs6_lr1e-4_bs4_gradacc8_lora_r8alpha16dropout0.05/checkpoint-551/R2AVSBenchinference_cot_ref_avs_test_s_bs/inference_results.jsonl"
# inference results from Ref-Thinker, the correponding data are from Ref-AVSBench (but using the same test videos as R^2-AVSBench)
# JSONL_PATH = "epochs6_lr1e-4_bs4_gradacc8_lora_r8alpha16dropout0.05/checkpoint-551/R2AVSBenchButusingOriRef_inference_cot_ref_avs_test_u_bs/inference_results.jsonl"
print(JSONL_PATH)

TEST_ORI_REF = False #! not modify
...
if TEST_ORI_REF:
    META_CSV_PATH = "./R2AVSBench/RefAVSBench_metadata.csv"   
else:  # here
...
BOX_THRESHOLD = 0.1 
TEXT_THRESHOLD = 0.25
USE_F_OBJECT = False #! may change
SAVE_PRED_MASK = False #! may change
COUNT_VIDEO = 0
if USE_F_OBJECT:
    OUTPUT_DIR = os.path.join(os.path.dirname(JSONL_PATH), "video_batch_infer_f_object")
    metric_log_path = os.path.join(os.path.dirname(JSONL_PATH), f"f_object_eval_results_v2_boxthre{BOX_THRESHOLD}_textthre{TEXT_THRESHOLD}_reproduce.json")
else:
    OUTPUT_DIR = os.path.join(os.path.dirname(JSONL_PATH), "video_batch_infer_s_object")
    metric_log_path = os.path.join(os.path.dirname(JSONL_PATH), f"s_object_eval_results_v2_boxthre{BOX_THRESHOLD}_textthre{TEXT_THRESHOLD}_reproduce.json")
...
for item in tqdm(results, desc="Processing videos"):
    uid = item["uid"]
