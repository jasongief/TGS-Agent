...
def metric_s_for_null_batch(pred):
    # pred: N, h, w  (0 或 1)
    assert len(pred.shape) == 3
    
    if pred.dtype != torch.int:
        pred = pred.int()

    num_pixels_per_mask = pred.size(-1) * pred.size(-2) # 

    x = torch.sum(pred.view(pred.size(0), -1), dim=1) 
    
    s_values = torch.sqrt(x.float() / num_pixels_per_mask)
...
def mask_iou(pred, target, eps=1e-7):
    """
    Compute mean IoU for a batch of binary masks.

    Args:
        pred: Tensor of shape [N, H, W], dtype=bool or uint8 (binary mask)
        target: Tensor of shape [N, H, W], same shape and dtype
        eps: Small epsilon to prevent division by zero

    Returns:
        Mean IoU over N masks
    """
    ...
    no_obj_flag = (target.sum(dim=(1, 2)) == 0)

    inter = (pred & target).sum(dim=(1, 2))
    union = (pred | target).sum(dim=(1, 2))

    inter_no_obj = ((~pred) & (~target)).sum(dim=(1, 2))
    inter[no_obj_flag] = inter_no_obj[no_obj_flag]
    union[no_obj_flag] = num_pixels

    ious = inter.float() / (union.float() + eps)
    # set_trace()
    return ious.mean()


def ori_mask_iou(pred, target, eps=1e-7, size_average=True):
    r"""
        param: 
            pred: size [N x H x W]
...
def _eval_pr_binary(y_pred, y, cuda_flag=False):
    """
    计算二值（0或1）预测和真实标签的查准率和查全率。
    假设 y_pred 和 y 都已经是二值张量（只包含0和1）。
    """
    if cuda_flag:
        y_pred = y_pred.cuda()
        y = y.cuda()
    ...
    prec = tp / (y_temp.sum() + 1e-20)

    recall = tp / (y.sum() + 1e-20)
...
def Eval_Fmeasure(pred, gt, pr_num=255):
    ...
    for img_id in range(N):
        ...
        # prec, recall = _eval_pr_binary(pred[img_id], gt[img_id]) 
