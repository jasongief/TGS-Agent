...
from utils.constants import NUM_FRAMES, MAX_FRAMES, NUM_FRAMES_PER_SECOND, MMODAL_INDEX_TOKEN, IMAGE_TOKEN_INDEX
...
def process_image(image_path, processor, aspect_ratio='pad', num_frames=NUM_FRAMES, image_grid=False):
    ...
    if aspect_ratio == 'pad':
        ...
        images = [Image.fromarray(f) for f in images]
        images = [f.resize((336,336),Image.BICUBIC) for f in images]
