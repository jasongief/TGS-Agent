...
class LanguageBindVideoTower(nn.Module):
    ...
    ############################################################
    def load_model(self):
        model = LanguageBindVideo.from_pretrained(self.video_tower_name,local_files_only=True)
...
class LanguageBindAudioTower(nn.Module):
    ...
    def load_model(self):
        # model = LanguageBindVideo.from_pretrained(self.video_tower_name,local_files_only=True)
        model = LanguageBindAudio.from_pretrained(self.audio_tower_name,local_files_only=True)
    ...
    def feature_select(self, video_forward_outs):
        ...
        image_features = video_forward_outs.hidden_states[self.select_layer]
        if self.select_feature == 'patch':
            image_features = image_features[:, 1:]
        elif self.select_feature == 'cls_patch':
            image_features = image_features
    ...
    def forward(self, videos):
        if type(videos) is list:
            video_features = []
            for video in videos:
                video_forward_out = self.audio_tower(video.to(device=self.device, dtype=self.dtype).unsqueeze(0), output_hidden_states=True)
                video_feature = self.feature_select(video_forward_out).to(video.dtype)
                video_features.append(video_feature)
        else:
            video_forward_outs = self.audio_tower(videos.to(device=self.device, dtype=self.dtype), output_hidden_states=True)
            video_features = self.feature_select(video_forward_outs).to(videos.dtype)
